...
function PointMassBallisticSolver() {
    ...
    function EnvironmentalFactors(temperature, pressure, humidity, windSpeed, windAngle) {
        ...
        let psat = 6107.8 * Math.exp( 7.27*T/T_kel)
      
        let pv = humidity * psat;
        let pd = (p - pv);
      

        this.rho = (pd * Md + pv * Mv)/(R * T_kel);
    ...
    function getCD(mach) {
        let  CDTable = CD_G7;
        let i_below = 0;
        let i_above = CDTable.length-1;

        // console.log("Looking for " + mach + " initial range : " + i_below + " to " + i_above);
        while(i_below < i_above - 1) {
            let candidate = Math.trunc((i_below + i_above)/2);
        ...
        return CDTable[i_below][1] + CDTable[i_below][2] * (mach - CDTable[i_below][0]);
    ...
    function findZeroDerivative(p0, p1, p2) {
        let A = ((p0.y - p2.y)*(p1.x - p2.x) - (p1.y - p2.y)*(p0.x - p2.x)) / 
                ((p0.x * p0.x - p2.x * p2.x) *(p1.x - p2.x) - (p1.x * p1.x - p2.x * p2.x)*(p0.x - p2.x));
    ...
    function main(ranges) {
        ...
        let angle_mrad = angle * 1000;
        let clicks     = angle_mrad / 0.07;
        console.log(`Zero angle is ${(angle_mrad).toFixed(2)} mrad or ${clicks.toFixed(1)} clicks`);
        

        let trajectory = getTrajectory(range+1, angle + 16*0.07/1000, muzzleVelocityMps, massKg, bc, sightHeightM, env);
        // let actualZero = getZeroDistance(trajectory);
